...
@Service
public class Payment ExecutionRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Payment ExecutionRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.payment execution.hosturl}")
	private String url;
	
	@Value("${spring.api.payment execution.context-path}")
	private String contextPath;

	@Value("${spring.api.payment execution.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Payment ExecutionRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Payment ExecutionRestProcessor.class.getName());
