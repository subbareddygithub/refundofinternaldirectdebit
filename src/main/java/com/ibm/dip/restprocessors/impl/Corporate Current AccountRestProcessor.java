...
@Service
public class Corporate Current AccountRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Corporate Current AccountRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.corporate current account.hosturl}")
	private String url;
	
	@Value("${spring.api.corporate current account.context-path}")
	private String contextPath;

	@Value("${spring.api.corporate current account.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Corporate Current AccountRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Corporate Current AccountRestProcessor.class.getName());
