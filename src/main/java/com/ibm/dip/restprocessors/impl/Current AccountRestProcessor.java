...
@Service
public class Current AccountRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Current AccountRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.current account.hosturl}")
	private String url;
	
	@Value("${spring.api.current account.context-path}")
	private String contextPath;

	@Value("${spring.api.current account.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Current AccountRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Current AccountRestProcessor.class.getName());
